...
const dictionaries: Record<LangType, () => Promise<IDictionary>> = {
  uk: () => import('../dictionaries/uk.json').then(module => module.default),
  en: () => import('../dictionaries/en.json').then(module => module.default),
};

export const getDictionary = async (locale: LangType): Promise<IDictionary> => {
  // Список дозволених локалей
  const allowedLocales: LangType[] = ['uk', 'en'];
